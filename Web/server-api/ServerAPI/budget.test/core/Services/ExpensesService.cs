...
namespace budget.test.core {
  public class ExpensesServiceTest {
    ...
    public void GetLastItem() {
      ...
      var entityService = new ExpensesService(EnumerableServiceMock.Object, PostingServiceMock.Object, DeletingServiceMock.Object, FilteringServiceMock.Object);
      ...
      EnumerableServiceMock.Setup(x => x.GetItemViews(It.IsAny<AppUser>(), It.IsAny<string>(), It.IsAny<TableLazyLoadEvent>())).Returns<AppUser, string, TableLazyLoadEvent>((appUser, collectionName, tableLazyLoadEvent) => {
        return new(new List<ItemView>() {
          new ItemView(new Item() { Quantity = 10, IsRegular = true, Category = "credit" }),
        }, 1);
      });
      ...
      Assert.Equal("credit", result.Category);
    }
    ...
    public void GetRegularPayments() {
      ...
      EnumerableServiceMock.Setup(x => x.GetItemViews(It.IsAny<AppUser>(), It.IsAny<string>(), It.IsAny<TableLazyLoadEvent>())).Returns<AppUser, string, TableLazyLoadEvent>((appUser, collectionName, tableLazyLoadEvent) => {
        return new(new List<ItemView>() {
          new ItemView(new Item() { Quantity = 10, IsRegular = true, Category = "entertainment", Date = new DateTime(2020, 01, 01) }),
          new ItemView(new Item() { Quantity = 10, IsRegular = true, Category = "entertainment", Date = new DateTime(2020, 02, 01) }),
          new ItemView(new Item() { Quantity = 10, IsRegular = true, Category = "entertainment", Date = new DateTime(2020, 03, 01) }),
          new ItemView(new Item() { Quantity = 10, IsRegular = true, Category = "entertainment", Date = new DateTime(2020, 04, 01) }),
        }, 3);
      });
